...
public class OmsBoxPlugin extends AbstractUIPlugin {
    ...
    public static final String OMSBOX_ENCODING = "OMSBOX_ENCODING"; //$NON-NLS-1$
    ...
    public String retrieveSavedEncoding() {
        IPreferenceStore preferenceStore = OmsBoxPlugin.getDefault().getPreferenceStore();
        String savedEncodingLevel = preferenceStore.getString(OMSBOX_ENCODING);
        if (savedEncodingLevel.length() == 0) {
            savedEncodingLevel = "";
        }
        return savedEncodingLevel;
    }
    
    public void saveEncoding( final String encoding ) {
        IPreferenceStore preferenceStore = OmsBoxPlugin.getDefault().getPreferenceStore();
        preferenceStore.setValue(OMSBOX_ENCODING, encoding);
    }
    ...
    public String getClasspathJars() throws Exception {
        try {
            ...
            // add udig libs
            // FIXME need to remove this, don't want to have plugin-internal libs
            // Bundle udigLibsBundle = Platform.getBundle("eu.udig.omsbox");
    ...
    private void addCustomLibs( StringBuilder sb ) throws IOException {
        ...
        File installFolder = new File(path);// DataUtilities.urlToFile(installLocation.getURL());
    ...
    /**
     * Get the spatialtoolbox libs folder in rthe installation folder.
     * 
     * @return the folder or <code>null</code>.
     */
    public static File getLibsFolders() {
        Location installLocation = Platform.getInstallLocation();
        String path = installLocation.getURL().getPath();
        File installFolder = new File(path);
        if (installFolder != null && installFolder.exists()) {
            File omsboxLibsFolder = new File(installFolder, "spatialtoolbox/libs");
    ...
    /**
     * Get the spatialtoolbox modules folder in the installation folder.
     * 
     * @return the folder or <code>null</code>.
     */
    public static File getModulesFolders() {
        Location installLocation = Platform.getInstallLocation();
        String path = installLocation.getURL().getPath();
        File installFolder = new File(path);
        if (installFolder != null && installFolder.exists()) {
            File omsboxLibsFolder = new File(installFolder, "spatialtoolbox/modules");
            if (omsboxLibsFolder.exists()) {
                return omsboxLibsFolder;
            }
        }
    ...
    // "unable to locate processing region decorator", eek);
