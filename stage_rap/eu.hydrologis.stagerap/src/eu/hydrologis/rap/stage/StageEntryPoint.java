...
//		infoColumn.setLayoutData(ExampleUtil.createFillData());
	...
	private Control createLogColumn(Composite parent) {
		...
		logText = new Text(column, SWT.MULTI | SWT.READ_ONLY | SWT.H_SCROLL
				| SWT.BORDER);
	...
	private Control createFileUploadArea(Composite parent) {
		...
		uploadButton.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false));
	...
	private void openFileDialog(Shell parent, boolean multi) {
		final FileDialog fileDialog = new FileDialog(parent,
				getDialogStyle(multi));
		fileDialog.setText(multi ? "Upload Multiple Files"
				: "Upload Single File");
