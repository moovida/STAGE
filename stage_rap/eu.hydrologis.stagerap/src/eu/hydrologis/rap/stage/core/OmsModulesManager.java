...
public class OmsModulesManager {
	...
	public TreeMap<String, List<ModuleDescription>> browseModules(boolean rescan) {
		...
		Set<Entry<String, List<ModuleDescription>>> entrySet = modulesMap
				.entrySet();
		for (Entry<String, List<ModuleDescription>> entry : entrySet) {
			Collections.sort(entry.getValue(),
					new ModuleDescription.ModuleDescriptionNameComparator());
		}
	...
	private void scanForModules() throws Exception {
		...
		for (Class<?> moduleClass : classesList) {
			try {
				...
				Description description = moduleClass
				...
				ModuleDescription module = new ModuleDescription(moduleClass,
						categoryStr, descrStr, status);
	...
	private void addInput(Access access, ModuleDescription module)
			throws Exception {
		...
		if (descriptionAnn != null) {
			descriptionStr = AnnotationUtilities
					.getLocalizedDescription(descriptionAnn);
		}
		...
		module.addInput(fieldName, fieldClass.getCanonicalName(),
				descriptionStr, defaultValue, uiHint);
	}

	private void addOutput(Access access, ModuleDescription module)
			throws Exception {
		Field field = access.getField();
		Description descriptionAnn = field.getAnnotation(Description.class);
		String descriptionStr = "No description available";
		...
		module.addOutput(fieldName, fieldClass.getCanonicalName(),
				descriptionStr, defaultValue, uiHint);
	}
	 ...
	 */
	public Class<?> getModulesClass(String className)
			throws ClassNotFoundException {
