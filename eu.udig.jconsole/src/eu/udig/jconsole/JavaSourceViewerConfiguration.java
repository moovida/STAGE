...
public class JavaSourceViewerConfiguration extends SourceViewerConfiguration {
	...
	public String getConfiguredDocumentPartitioning(ISourceViewer sourceViewer) {
		return JConsoleEditorPlugin.JAVA_PARTITIONING;
	}
	...
	public IContentAssistant getContentAssistant(ISourceViewer sourceViewer) {
		...
		assistant.setContextInformationPopupBackground(JConsoleEditorPlugin.getDefault().getJavaColorProvider().getColor(new RGB(150, 150, 0)));
	...
	public IPresentationReconciler getPresentationReconciler(ISourceViewer sourceViewer) {
		...
		JavaColorProvider provider= JConsoleEditorPlugin.getDefault().getJavaColorProvider();
		PresentationReconciler reconciler= new PresentationReconciler();
		reconciler.setDocumentPartitioning(getConfiguredDocumentPartitioning(sourceViewer));

		DefaultDamagerRepairer dr= new DefaultDamagerRepairer(JConsoleEditorPlugin.getDefault().getJavaCodeScanner());
		reconciler.setDamager(dr, IDocument.DEFAULT_CONTENT_TYPE);
		reconciler.setRepairer(dr, IDocument.DEFAULT_CONTENT_TYPE);

		dr= new DefaultDamagerRepairer(JConsoleEditorPlugin.getDefault().getJavaDocScanner());
